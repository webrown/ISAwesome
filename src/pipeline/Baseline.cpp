...
Status Baseline::run(void){
    ...
    int opcode = spliceMachineCode(nextInstruction, 22, 27);
    ...
    // What is the flag?
    int flagCode = spliceMachineCode(nextInstruction, 28, 31);
    Conditional flag;
    ...
    switch(instructionType) {
        case ADD:
            ...
            addThis.push_back(binaryOperand1);
            if(!useImmediate) {
            ...
            if(addThis.size() == destinationContents.size()) {
                ...
                for(int i = 0; i < addThis.size(); i++) {
                    destinationContents[i] += addThis[i];
                }
            }
            else if(addThis.size() == 1){
                // scalar to vector incrememtation.
                for(int i = 0; i < addThis.size(); i++) {
                    destinationContents[i] += addThis[0];
                }
            }
            // Write back
        ...
        case CMP:
            ...
            if(arg1.size() == arg2.size()) {
                ...
                for(int i = 0; i < arg1.size(); i++) {
                    registers->write(arg1[i] == arg2[i], Register::flags+i);//TODO 0
                    registers->write(arg1[i] != arg2[i], Register::flags+i);//TODO 1
                    registers->write(arg1[i] <= arg2[i], Register::flags+i);//TODO 10
                    registers->write(arg1[i] >= arg2[i], Register::flags+i);//TODO 11
                    registers->write(arg1[i] < arg2[i], Register::flags+i);//TODO 12
                    registers->write(arg1[i] > arg2[i], Register::flags+i);//TODO 13
                }
            }
            else if(arg1.size() == 1){
                // scalar to vector incrememtation.
                for(int i = 0; i < arg1.size(); i++) {
                    registers->write(arg1[0] == arg2[i], Register::flags+i);//TODO 0
                    registers->write(arg1[0] != arg2[i], Register::flags+i);//TODO 1
                    registers->write(arg1[0] <= arg2[i], Register::flags+i);//TODO 10
                    registers->write(arg1[0] >= arg2[i], Register::flags+i);//TODO 11
                    registers->write(arg1[0] <  arg2[i], Register::flags+i);//TODO 12
                    registers->write(arg1[0] >  arg2[i], Register::flags+i);//TODO 13
                }
            }
            break;
        case CPY:
            QVector<Value> addThis;
            placeThis.push_back(binaryOperand1);
            if(!useImmediate) {
                qr = registers.read(binaryOperand1);
                placethis = qr->result;
                delete qr;
        ...
        case LOD:
            ...
            memAddr.push_back(binaryOperand1);
            if(!useImmediate) {
                qr = registers.read(binaryOperand1);
                memAddr = qr->result;
                delete qr;
            ...
            qr = memory->read(memoryAddr, regContents.size());
        ...
        case STO:
            ...
            memory->write(regContents, memAddr);
            break;
    }
